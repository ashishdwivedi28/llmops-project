...
def list_rag_documents(project_id: str, app_id: str, location: str = "us-west1"):
    ...
    if not corpus_name:
        logger.error(
            f"No 'rag_corpus_id' found for app '{app_id}'. Run setup_rag_corpus.py first."
        )
        return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="List documents in Vertex AI RAG Corpus"
    )
    parser.add_argument("--project", required=True, help="GCP Project ID")
    parser.add_argument(
        "--app_id", default="rag_bot", help="App ID in Firestore (default: rag_bot)"
    )
    parser.add_argument(
        "--location", default="us-west1", help="GCP Region (must match corpus location)"
    )
